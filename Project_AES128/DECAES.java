public class DECAES{
	static String MulM[][]={{"0E","0B","0D","09"},{"09","0E","0B","0D"},{"0D","09","0E","0B"},{"0B","0D","09","0E"}};
	...
	public static String[][] SBoxI={{"52","09","6A","D5","30","36","A5","38","BF","40","A3","9E","81","F3","D7","FB"},
	...
	public static String  mixCol(String state){
		...
		for(int i=0;i<4;++i){
			for(int j=0;j<4;++j){
				...
				for(int k=0;k<4;++k){
					if(k==0) {
						////System.out.println("st and mulm:"+st[k][i]+" "+MulM[j][k]);
    ...
    {	String result="";
        ...
        result+=SBoxI[row][col];
       
    	}
    	 //System.out.println("After sub byte:"+result);
        return result;
    }
	public static String SubWord2(String state)
    {	String result="";
    	int row,col;
    	for(int i=0;i<state.length();i+=2){
        row=Integer.parseInt(""+state.charAt(i),16);
	...
	public static void expandKey(){
		...
		for(int i=0;i<44;++i){
			...
			if(i==0){
				...
				temp=Long.toHexString(Long.parseLong(SubWord2(RotWord(EK((i-1)*4))),16)^ Long.parseLong(Rcon((i/4)-1),16)^Long.parseLong(EK((i-4)*4),16));
				//padd with zero
				while(temp.length()<8){
					temp="0"+temp;
				}
	...
	public static String decrypt(String plaintext){
		...
		for(int i=9;i>=0;--i){
			if(i==0){
				state=(addRoundKey(SubWord(ShiftRow(state)),i));
			}
			else
				state=mixCol(addRoundKey(SubWord(ShiftRow(state)),i));
		}
		return state;
	}
	public static void main(String[] args) {
		String cipher="5a722078545e1060975b80680198aa18";;
		System.out.println("Plain text:"+decrypt(cipher));
	}
}	